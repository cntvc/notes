...
## Output
14583 INFO: Updating manifest in D:\Github\Genshin\Genshin-Impact-Tools\dist\Genshin_Impact_Tools.exe.notanexecutable
14583 INFO: Updating resource type 24 name 1 language 0
14586 INFO: Appending PKG archive to EXE
14591 INFO: Fixing EXE headers
...
- 将所有的源代码文件进行打包
- 设置 exe 文件名为 `Genshin_Impact_Tools`
- 设置程序图标
- 将默认的配置文件作为资源文件打包
...
# 默认为 py 格式的配置文件
global_config = "genshin/config/global_setting.py"
...
建议在虚拟环境中进行打包，**注意**：如果你在全局环境安装了pyinstaller，在程序的虚拟环境也安装了pyinstaller包，那么在虚拟环境中运行打包命令时，仍然使用的是全局环境的python，这会导致一些无关的包被捆绑进来

这里示例，使用 `pipenv` 工具进行虚拟环境管理时，如何调用虚拟环境打包

```powershell
pipenv run pyinstaller --clean main.spec
```

### 2. 打包后提示找不到资源文件

> 相关链接
[pyinstaller 运行时信息](https://pyinstaller.org/en/stable/runtime-information.html)
...
**原因分析**: 打包单文件后，Windows系统运行时会对文件解压到临时目录，资源文件目录在临时文件夹，此时运行目录为exe程序所在目录，而资源访问的相对路径却是在临时文件夹下，如果代码中使用了类似 `os.getcwd()` 的代码访问资源文件，就会提示找不到文件。

**第一种方案**:

在前面的项目中，`config` 模块需要读取 `global_setting.py`  中的配置，因此在打包时将  `global_setting.py` 文件作为资源文件打包到目录 `genshin/config` 下。此时在运行时解压的资源文件路径为 
...
```python
path = getattr(sys, "_MEIPASS") / "genshin/config/global_setting.py"
```

**第二种解决方案：**

使用 `__file__` ，在打包单文件捆绑时，PyInstaller 引导加载程序会将模块的 `__file__` 属性设置为相对于捆绑文件夹的正确路径。即 `__file__ = getattr(sys, "_MEIPASS") + relative_path`

在 `config/__init__.py` 中初始化配置路径
...
## 四、巨人的肩膀
[Pyinstaller的Spec文件用法_tangfreeze的博客](https://blog.csdn.net/tangfreeze/article/details/112240342)
